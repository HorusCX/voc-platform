...
echo -e "${YELLOW}Registering voc-worker-task...${NC}"
aws ecs register-task-definition \
...
WORKER_TASK_DEF=$(aws ecs describe-task-definition --task-definition voc-worker-task --query 'taskDefinition.taskDefinitionArn' --output text)

echo -e "\n${YELLOW}Latest API Task Definition: $API_TASK_DEF${NC}"
echo -e "${YELLOW}Latest Worker Task Definition: $WORKER_TASK_DEF${NC}"
...
echo -e "${YELLOW}Updating voc-worker-service...${NC}"
aws ecs update-service \
    --cluster "$CLUSTER_NAME" \
    --service voc-worker-service \
...
# Get running tasks for Worker service
WORKER_TASKS=$(aws ecs list-tasks \
    --cluster "$CLUSTER_NAME" \
...
# Stop each Worker task
if [ -n "$WORKER_TASKS" ]; then
    echo -e "\n${YELLOW}Stopping Worker service tasks...${NC}"
    ...
    echo -e "${GREEN}✅ Worker tasks stopped${NC}"
else
    echo -e "${YELLOW}No running Worker tasks to stop${NC}"
fi
...
echo -e "\n${YELLOW}Worker Service Status:${NC}"
aws ecs describe-services \
    --cluster "$CLUSTER_NAME" \
    --services voc-worker-service \
    --region "$REGION" \
    --query 'services[0].deployments[0].[status,runningCount,desiredCount,taskDefinition]' \
    --output table
...
echo "1. Monitor CloudWatch logs at /ecs/voc-api and /ecs/voc-worker"
